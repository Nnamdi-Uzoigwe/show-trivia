...
//                 {[
//                   "20 random questions are picked from a pool of 100 each attempt",
...
export default function ShowPreviewPage() {
  ...
  useEffect(() => {
    if (!user) return;
    fetch(`/api/attempts/check?userId=${user._id}&showSlug=${slug}`)
      .then((r) => r.json())
      .then(setAttemptInfo);
  }, [user, slug]);
  ...
  const canPlay = !attemptInfo || attemptInfo.canAttempt;
        ...
        <div className="max-w-4xl mx-auto px-6 -mt-16 relative z-10 pb-24">
          ...
          >
              ...
              >
                <div className="flex flex-col gap-0.5">
                  <p
                    ...
                    style={{ color: "var(--text-primary)" }}
                  >
                    {canPlay ? "You can play this quiz" : "Limit reached for today"}
                  </p>
                  <p style={{ color: "var(--text-muted)", fontSize: "13px" }}>
                    {attemptInfo.remainingShowAttempts} of 4 show attempts left ·{" "}
                    {attemptInfo.remainingTotalAttempts} of 20 daily attempts left
                  </p>
                </div>
            ...
            >
              ...
              <ul className="flex flex-col gap-2">
                {[
                  "20 random questions picked from a pool of 100 each attempt",
                  "You have 30 minutes — timer starts the moment you begin",
                  "You can attempt this quiz up to 4 times per day",
                  "Max 20 total quiz attempts across all shows per day",
                  "Your best score goes on the leaderboard",
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <button
                className="btn-primary w-full sm:w-auto px-12 py-4 text-base"
                onClick={() => canPlay && router.push(`/quiz/${slug}/play`)}
                disabled={!canPlay}
